...
public class FBO {
	...
	public static final class ScreenFBO extends FBO {
		...
		public static final int DEFAULT_DRAW_BUFFER = IS_DOUBLE_BUFFERED ? GL_BACK_LEFT : GL_FRONT_LEFT;
	...
	public void attach(IRenderTarget target, Attachment attachment, int index) {
		...
		if(target.getFormat() == null && FBOS_CAN_INITALISE_TARGETS) 
			target.init(attachment.getFormat());
				...
				!(attachment.getFormat() == TargetFormat.Colour && target.getFormat() == TargetFormat.ColourData)) 
...
//			array[index].cleanUp();
	...
	public FBO resolve(FBO dest, int srcIndex, int destIndex) {					// Resolve selected to FBO at selected
		...
		if(srcIndex == -1) 		 bindRead(); else 	   bindRead(srcIndex); 
		if(destIndex == -1) dest.bindDraw(); else dest.bindDraw(destIndex); 
	...
	public int[] getDrawBuffers() { return drawBuffersRaw; }
